...
if ~isfolder(Data_Set)
  Alert_user = sprintf('No Folder exists in Directory: %s', Data_Set);
  uiwait(warndlg(Alert_user));
  return;
end

% Read all files with .jpg extension
File_Ext = fullfile(Data_Set, '*.jpg');
File_instance = dir(File_Ext);

for k = 1 : 2
F_name = File_instance(k).name;
fprintf(1, 'Performing Text Extraction on: %s', F_name);
...
bw = rgb2gray(image);                   %bw=black and white image
figure(2)
imshow(bw)
